// Modified By: Tarnveer Takhtar (takhtart) to work with PCL Versions 1.14.1+
// Original Author: Tsukasa Sugiura (UnaNancyOwen)
// https://github.com/UnaNancyOwen/KinectGrabber/blob/Kinect2Grabber/kinect2_grabber.h
// Kinect2Grabber is pcl::Grabber to retrieve the point cloud data from Kinect
// v2 using Kinect for Windows SDK 2.x. This source code is licensed under the
// MIT license.
...
class Kinect2Grabber : public pcl::Grabber {
 ...
 protected:
  ...
  pcl::PointCloud<pcl::PointXYZI>::Ptr convertInfraredDepthToPointXYZI(
      UINT16* infraredBuffer, UINT16* depthBuffer);
  pcl::PointCloud<pcl::PointXYZRGB>::Ptr convertRGBDepthToPointXYZRGB(
      RGBQUAD* colorBuffer, UINT16* depthBuffer);
...
pcl::Kinect2Grabber::Kinect2Grabber()
  ...
  if (FAILED(result)) {
    throw std::exception(
        "Exception : IKinectSensor::get_InfraredFrameSource()");
  }
  ...
  if (FAILED(result)) {
    throw std::exception(
        "Exception : IColorFrameSource::get_FrameDescription()");
  }
  ...
  if (FAILED(result)) {
    throw std::exception(
        "Exception : IDepthFrameSource::get_FrameDescription()");
  }
  ...
  if (FAILED(result)) {
    throw std::exception(
        "Exception : IInfraredFrameSource::get_FrameDescription()");
  }
...
float pcl::Kinect2Grabber::getFramesPerSecond() const { return 30.0f; }
  ...
  while (!quit) {
    ...
    if (SUCCEEDED(result)) {
      ...
      result = colorFrame->CopyConvertedFrameDataToArray(
          colorBuffer.size() * sizeof(RGBQUAD),
          reinterpret_cast<BYTE*>(&colorBuffer[0]),
          ColorImageFormat::ColorImageFormat_Bgra);
      if (FAILED(result)) {
        throw std::exception(
            "Exception : IColorFrame::CopyConvertedFrameDataToArray()");
      }
    ...
    if (SUCCEEDED(result)) {
      ...
      result = infraredFrame->CopyFrameDataToArray(infraredBuffer.size(),
                                                   &infraredBuffer[0]);
      if (FAILED(result)) {
        throw std::exception(
            "Exception : IInfraredFrame::CopyFrameDataToArray()");
      }
    }
    ...
    if (signal_PointXYZI->num_slots() > 0) {
      signal_PointXYZI->operator()(
          convertInfraredDepthToPointXYZI(&infraredBuffer[0], &depthBuffer[0]));
    }

    if (signal_PointXYZRGB->num_slots() > 0) {
      signal_PointXYZRGB->operator()(
          convertRGBDepthToPointXYZRGB(&colorBuffer[0], &depthBuffer[0]));
    }

    if (signal_PointXYZRGBA->num_slots() > 0) {
      signal_PointXYZRGBA->operator()(
...
pcl::PointCloud<pcl::PointXYZ>::Ptr pcl::Kinect2Grabber::convertDepthToPointXYZ(
  ...
  for (int y = 0; y < depthHeight; y++) {
    for (int x = 0; x < depthWidth; x++, pt++) {
      ...
      DepthSpacePoint depthSpacePoint = {static_cast<float>(x),
                                         static_cast<float>(y)};
      UINT16 depth = depthBuffer[y * depthWidth + x];

      // Coordinate Mapping Depth to Camera Space, and Setting PointCloud XYZ
      CameraSpacePoint cameraSpacePoint = {0.0f, 0.0f, 0.0f};
      mapper->MapDepthPointToCameraSpace(depthSpacePoint, depth,
                                         &cameraSpacePoint);
...
pcl::Kinect2Grabber::convertInfraredDepthToPointXYZI(UINT16* infraredBuffer,
                                                     UINT16* depthBuffer) {
...
pcl::Kinect2Grabber::convertRGBDepthToPointXYZRGB(RGBQUAD* colorBuffer,
                                                  UINT16* depthBuffer) {
  ...
  for (int y = 0; y < depthHeight; y++) {
    for (int x = 0; x < depthWidth; x++, pt++) {
      ...
      mapper->MapDepthPointToColorSpace(depthSpacePoint, depth,
                                        &colorSpacePoint);
      int colorX = static_cast<int>(std::floor(colorSpacePoint.X + 0.5f));
      int colorY = static_cast<int>(std::floor(colorSpacePoint.Y + 0.5f));
      if ((0 <= colorX) && (colorX < colorWidth) && (0 <= colorY) &&
          (colorY < colorHeight)) {
...
pcl::Kinect2Grabber::convertRGBADepthToPointXYZRGBA(RGBQUAD* colorBuffer,
                                                    UINT16* depthBuffer) {
