...
\item{Linting (NFT22)\label{NFT22}\\}
...
\subsubsection{Module 2D Filtering}

The Test used to cover 2D Filtering is to ensure that the data transferred from the Kinect Sensor is correct to ensure that any filtering done will be done on a correct cv::Mat.
...
\subsubsection{Module 3D Cluster Extraction}

There are two tests here that will cover the 3D Cluster Extraction module one with a valid cloud and one with an invalid cloud. These two tests will cover the main scope of the clustering to ensure the module runs as expected.
...
\subsubsection{Module 3D Filtering}

The two tests below will cover the 3D filtering module by covering the main filtering methods of the module. Testing to ensure that voxel downsampling and noise removal is functioning properly will ensure that the cloud is filtered before the subsequent processing methods are run.
